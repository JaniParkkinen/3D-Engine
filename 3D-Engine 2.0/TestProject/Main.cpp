...
public:
	...
	virtual void Update( Engine::DeltaTime deltaTime ) override {
		...
		if ( keyboard.getKey( 'A' ) ) {
			turn = true;
			Engine::EntityManager::GetInstance( )->GetComponent<Engine::Physics>( "Player", PHYS )->AddAngularVelocity( glm::vec3( 0.0f, 2.5f, 0.0f ) );
			//Engine::EntityManager::GetInstance( )->GetComponent<Engine::Transform>( "Player", TRANSFORM )->RotateWithoutChildren( glm::vec3( 0.0f, 2.5f, 0.0f ) );
		}
		...
		if ( keyboard.getKeyDown( VK_SPACE ) ) {
			//Jump
			Engine::EntityManager::GetInstance( )->GetComponent<Engine::Physics>( "Player", PHYS )->AddVelocity( glm::vec3( 0.0f, 50.0f, 0.0f ) );
		}

		Engine::EntityManager::GetInstance( )->GetComponent<Engine::Physics>( "Player", PHYS )->SetVelocity( Engine::EntityManager::GetInstance( )->GetComponent<Engine::Physics>( "Player", PHYS )->GetVelocity( ) * 0.9f );
		if ( !turn ) {
...
public:
	...
	virtual ~Exit( ) { }
	virtual void Init( ) override {
		Engine::EntityManager::GetInstance( )->AddComponent<Engine::Material>( _name, std::make_shared<Engine::Material>( Engine::ResourceManager::GetInstance( )->LoadResource( "Assets/Exit.obj" )->getMaterial( ) ) );
		Engine::EntityManager::GetInstance( )->AddComponent<Engine::Render>( _name, std::make_shared<Engine::Render>( Engine::ResourceManager::GetInstance( )->LoadResource( "Assets/Exit.obj" )->getShapes( ) ) );
